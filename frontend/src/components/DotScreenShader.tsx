...
const DotMaterial = shaderMaterial(
  {
    ...
    dotColor: new THREE.Color("#94a3b8"),
    bgColor: new THREE.Color("#fef3c7"),
    mouseTrail: null,
    render: 0,
    rotation: 0,
    gridSize: 50,
  ...
  /* glsl */ `
    ...
    uniform float enableMouseTrail;
    ...
    void main() {
      ...
      // Screen mask - subtle gradient
      float screenMask = smoothstep(0.0, 0.3, 1.0 - uv.y);
      ...
      // Mouse trail effect
      float mouseInfluence = texture2D(mouseTrail, gridUvCenterInScreenCoords).r * enableMouseTrail;
...
function Scene({
  ...
  useEffect(() => {
    ...
    dotMaterial.uniforms.bgColor.value.setHex(
      Number(bgColor.replace("#", "0x"))
    );
    dotMaterial.uniforms.dotOpacity.value = dotOpacity;
    dotMaterial.uniforms.enableMouseTrail.value = enableMouseTrail ? 1.0 : 0.0;
  }, [dotMaterial, bgColor, dotColor, dotOpacity, enableMouseTrail]);
